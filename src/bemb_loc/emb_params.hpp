...
public:
	...
	string label;
  	bool autoLabel;
	int IC;							// #observed item attributes
	int UC;							// #observed user attributes
	bool noItemPriceLatents;
	std::vector<std::pair<int,int>> ICgroups;  // groups of observed attributes
	std::vector<std::pair<int,int>> ICeffects; // how each group of observed attributes affects latent vectors
	std::vector<std::pair<int,int>> ICeffects_price; // how each group of observed attributes affects latent price vectors
	bool flag_itemIntercept;
	int flag_step_schedule;		// step_schedule: 0=advi, 1=rmsprop, 2=adagrad
	int flag_likelihood;			// likelihood: 0=bernoulli, 1=one-vs-each, 2=regularized softmax, 3=within-group softmax, 4=softmax
	int flag_shuffle;				// shuffle: -1=conditionally specified [option disabled], 0=don't, 1+=yes
	...
	double flag_normPrice_val;
	double flag_priceThreshold;
	bool flag_checkout;
	bool flag_obs2utility;
	bool flag_obs2prior;
	/* int flag_lookahead; */
	double eta;
	double gamma;
	...
	my_param() {
		...
		IC = 0;
		UC = 0;
		noItemPriceLatents = false;
		seed = 0;
		rfreq = 10;
		saveCycle = 100;
		printInitMatrixVal = false;
		negsamples = 10;
		zeroFactor = 0.1;
		batchsize = -1;
		Niter = 1500;
		flag_itemIntercept = false;
		flag_step_schedule = 2;
		flag_nsFreq = -1;
		flag_likelihood = 0;
		flag_shuffle = 1;
		flag_userVec = 0;
		...
		flag_normPrice_val = -1;
		flag_priceThreshold = 0.0;
		flag_checkout = false;
		flag_obs2utility = false;
		flag_obs2prior = false;
		/* flag_lookahead = 0; */
		label = "";
    		autoLabel = true;
		eta = 0.1;
...
public:
	...
	double mean_gamma;
	double s2gamma;
	double mean_gammaObsItem;
	double s2gammaObsItem;
	double mean_beta;
	double s2beta;
	double s2H;
	double s2obsPrior;
...
public:
	...
	// Observed attributes
	Matrix2D<double> attr_items;	// Observed attributes for items
	Matrix2D<double> attr_users;	// Observed attributes for users
	...
	// Lists for transactions in validation set (same definition as above, but for validation set only)
	int val_Ntrans;
	Matrix1D<std::vector<int>> val_lines_per_trans;
	Matrix1D<std::vector<int>> val_items_per_trans;
	Matrix1D<int> val_Nitems_per_trans;
	Matrix1D<int> val_user_per_trans;
	Matrix1D<int> val_session_per_trans;
	Matrix1D<std::vector<int>> val_sessions_per_user;
	std::map<hpf_trans_aux,int> val_trans_ids;			// Map containing the transaction id's (for test)
	...
	static inline unsigned long long find_by_value(const std::map<unsigned long long,int> &ids, int val) {
	...
	void create_transactions_val(const my_param &param) {
		if(param.noVal) {
			return;
		}
		...
		// Create valid_trans_ids
		val_Ntrans = 0;
		for(unsigned int t=0; t<obs_val.T; t++) {
			u = obs_val.y_user[t];
			s = obs_val.y_sess[t];
			hpf_trans_aux tid = hpf_trans_aux(u,s);
			if(val_trans_ids.find(tid)==val_trans_ids.end()) {
				val_trans_ids.insert(std::pair<hpf_trans_aux,int>(tid,val_Ntrans));
				val_Ntrans += 1;
			}
		}

		// Create val_sessions_per_user
		val_sessions_per_user = Matrix1D<std::vector<int>>(Nusers);
		for(auto const &it : val_trans_ids) {
			u = it.first.u;
			s = it.first.s;
			val_sessions_per_user.get_object(u).push_back(s);
		}

		// Create val_user_per_trans and val_session_per_trans
		val_user_per_trans = Matrix1D<int>(val_Ntrans);
		val_session_per_trans = Matrix1D<int>(val_Ntrans);
		for(auto const &it : val_trans_ids) {
			u = it.first.u;
			s = it.first.s;
			int t = it.second;
			val_user_per_trans.set_object(t,u);
			val_session_per_trans.set_object(t,s);
		}

		// Create val_lines_per_trans, val_items_per_trans, and obs_val.y_trans
		val_lines_per_trans = Matrix1D<std::vector<int>>(val_Ntrans);
		val_items_per_trans = Matrix1D<std::vector<int>>(val_Ntrans);
		val_Nitems_per_trans = Matrix1D<int>(val_Ntrans);
		set_to_zero(val_Nitems_per_trans);
		for(unsigned int ll=0; ll<obs_val.T; ll++) {
			u = obs_val.y_user[ll];
			s = obs_val.y_sess[ll];
			i = obs_val.y_item[ll];
			int t = val_get_transaction(u,s);
			if(t<0) {
				std::cerr << "[ERR] Transaction not found (my_data::create_transactions_val)" << endl;
				assert(0);
			}
			obs_val.y_trans[ll] = t;
			val_lines_per_trans.get_object(t).push_back(ll);
			val_items_per_trans.get_object(t).push_back(i);
			aux = val_Nitems_per_trans.get_object(t);
			aux += static_cast<int>(obs_val.y_rating[ll]);
			val_Nitems_per_trans.set_object(t,aux);
		}
	}
	...
	inline int val_get_transaction(int u, int s) {
		int t = -1;
		hpf_trans_aux tid = hpf_trans_aux(u,s);
		std::map<hpf_trans_aux,int>::const_iterator iter = val_trans_ids.find(tid);
		if(iter!=val_trans_ids.end()) {
			t = iter->second;
		}
		return t;
	}
...
public:
	...
	Matrix2D<var_gaussian> obsItems;  // latent factors corresponding to observed item attributes
	Matrix1D<var_gaussian> gammaObsItem; // latent factor corresponding to observed item log price
	Matrix2D<var_gaussian> obsUsers;  // latent factors corresponding to observed user attributes
	/* Matrix2D<double> sum_alpha; */     // auxiliary variable (sum of alpha's in the context)
	Matrix2D<var_gaussian> gamma;       // price sensitivity vectors (per-user)
	Matrix2D<var_gaussian> beta;	  	 // price sensitivity vectors (per-item)
	Matrix2D<var_gaussian> delta;	 // seasonal effects
	Matrix2D<var_gaussian> mu;		 // seasonal effects (per-item)
	Matrix2D<var_gaussian> weekdays; // weekday effects (Nitems x Nweekdays)
	...
	Matrix2D<double> prod_obsItems;			// products obsItems*data.attr_items
	Matrix2D<double> prod_obsUsers;			// products obsUsers*data.attr_users
	Matrix1D<double> eta_base;				// eta_base for all items
	Matrix2D<var_gaussian> H_alpha;			// Matrix for the prior over latent features
	Matrix2D<var_gaussian> H_beta;			// Matrix for the prior over latent price vectors
	Matrix2D<bool> validH_alpha;
	Matrix2D<bool> validH_beta;
	...
	my_pvar(const my_data &data, const my_param &param) {
		...
		if(param.IC>0 && param.flag_obs2utility) {
			obsItems = Matrix2D<var_gaussian>(data.Nusers, param.IC);
			prod_obsItems = Matrix2D<double>(data.Nusers, data.Nitems);
		}
		if(param.UC>0) {
			obsUsers = Matrix2D<var_gaussian>(data.Nitems, param.UC);
			prod_obsUsers = Matrix2D<double>(data.Nusers, data.Nitems);
		}
		if(param.IC>0 && param.flag_obs2prior) {
			H_alpha = Matrix2D<var_gaussian>(param.IC, param.K);
			validH_alpha = Matrix2D<bool>(param.IC, param.K);
			initialize_validH(validH_alpha, param.ICgroups, param.ICeffects);
		}
		...
		if(param.flag_price>0) {
			if (param.noItemPriceLatents) {
				gammaObsItem = Matrix1D<var_gaussian>(data.Nusers);
			} else {
				gamma = Matrix2D<var_gaussian>(data.Nusers, param.flag_price);
				beta = Matrix2D<var_gaussian>(data.Nitems, param.flag_price);
				prod_gamma_beta = Matrix2D<double>(data.Nusers,data.Nitems);
			}
		}
		if(param.flag_price>0 && param.IC>0 && param.flag_obs2prior) {
			H_beta = Matrix2D<var_gaussian>(param.IC, param.flag_price);
			validH_beta = Matrix2D<bool>(param.IC, param.flag_price);
			initialize_validH(validH_beta, param.ICgroups, param.ICeffects_price);
		}
		if(param.flag_day>0) {
			delta = Matrix2D<var_gaussian>(data.Ndays, param.flag_day);
			mu = Matrix2D<var_gaussian>(data.Nitems, param.flag_day);
			prod_delta_mu = Matrix2D<double>(data.Ndays,data.Nitems);
		}
		if(param.flag_weekdays) {
			weekdays = Matrix2D<var_gaussian>(data.Nitems, data.Nweekdays);
		}
		eta_base = Matrix1D<double>(data.Nitems);
	}
	...
	void initialize_validH(Matrix2D<bool> &validM, const std::vector<std::pair<int,int>> &ICgroups, \
						   const std::vector<std::pair<int,int>> &ICeffects) {
		if(ICgroups.size()==0) {
			set_to_one(validM);
		} else {
			set_to_zero(validM);
			for(unsigned int jj=0; jj<ICgroups.size(); jj++) {
				for(int ic=ICgroups.at(jj).first; ic<=ICgroups.at(jj).second; ic++) {
					for(int k=ICeffects.at(jj).first; k<=ICeffects.at(jj).second; k++) {
						validM.set_object(ic,k,true);
					}
				}
			}
		}
	...
	void initialize_all(gsl_rng *semilla, my_data &data, const my_param &param, const my_hyper &hyper) {
		...
		if(param.K>0) {
			if(param.iniFromGroup!="") {
				std::cerr << "[ERR] Not implemented: iniFromGroup" << endl;
				assert(0);
			  	//initialize_from_group_file(param.iniFromGroup+"/param_rho",data.item_ids,data.itemgroup_ids,data.group_per_item,rho,param,val_ini,false,semilla);
			} else if(param.iniPath=="") {
				// initialize randomly
				initialize_matrix_randomly(semilla,rho,val_ini,param.stdIni,false);
			} else {
				// initialize from file
					initialize_from_file(param.iniPath+"/param_rho",data.item_ids,rho,param,false);
			}
		}
		*/
		...
		if(param.K>0) {
			if(param.iniFromGroup!="") {
				std::cerr << "[ERR] Not implemented: iniFromGroup" << endl;
				assert(0);
				//initialize_from_group_file(param.iniFromGroup+"/param_alpha",data.item_ids,data.itemgroup_ids,data.group_per_item,alpha,param,val_ini,false,semilla);
			} else if(param.iniPath=="") {
				// initialize randomly
				initialize_matrix_randomly(semilla,alpha,val_ini,param.stdIni,false);
			} else {
				// initialize from file
				initialize_from_file(param.iniPath+"/param_alpha",data.item_ids,alpha,param,false);
			}
		}
		...
		if(param.IC>0 && param.flag_obs2utility) {
			if(param.iniPath=="") {
				// initialize randomly
				initialize_matrix_randomly(semilla,obsItems,val_ini,param.stdIni,false);
			} else {
				// initialize from file
				initialize_from_file(param.iniPath+"/param_obsItems",data.user_ids,obsItems,param,false);
			}
		}
		...
		if(param.UC>0) {
			if(param.iniPath=="") {
				// initialize randomly
				initialize_matrix_randomly(semilla,obsUsers,val_ini,param.stdIni,false);
			} else {
				// initialize from file
				initialize_from_file(param.iniPath+"/param_obsUsers",data.item_ids,obsUsers,param,false);
			}
		}
		...
		if(param.IC>0 && param.flag_obs2prior) {
			if(param.iniPath=="") {
				// initialize randomly
				initialize_matrix_randomly(semilla,H_alpha,val_ini,param.stdIni,false);
			} else {
				// initialize from file
				...
				initialize_from_file(param.iniPath+"/param_Halpha",aux_map,H_alpha,param,false);
			}
		}
		...
		if(param.flag_price>0 && param.IC>0 && param.flag_obs2prior) {
			if(param.iniPath=="") {
				// initialize randomly
				initialize_matrix_randomly(semilla,H_beta,val_ini,param.stdIni,false);
			} else {
				// initialize from file
				std::map<unsigned long long, int> aux_map;
				for(int k=0; k<param.IC; k++) {
					...
					initialize_from_file(param.iniPath+"/param_Hbeta",aux_map,H_beta,param,false);
			}
		}
		...
		if(param.flag_itemIntercept) {
			if(param.iniPath=="") {
				...
				// initialize from file
				initialize_from_file(param.iniPath+"/param_lambda0",data.item_ids,lambda0,param,false);
			}
		}
		...
		if(param.flag_price>0) {
			if(param.noItemPriceLatents) {
				if(param.flag_iniPriceVal or param.iniPath != "") {
					// BOMB
					std::cerr << ("-iniPath and -iniPriceVal not supported with -noItemPriceLatents");
					assert(0);
				} else {
					// initialize randomly
					initialize_matrix_randomly(semilla, gammaObsItem, val_ini, param.stdIni, false);
				} 
			} else {
				if(param.flag_iniPriceVal) {
					...
					for(int u=0; u<data.Nusers; u++) {
						for(int k=0; k<param.flag_price; k++) {
							gamma.get_object(u,k).initialize(param.iniPriceVal,false);
						}
					}
					for(int i=0; i<data.Nitems; i++) {
						for(int k=0; k<param.flag_price; k++) {
							beta.get_object(i,k).initialize(param.iniPriceVal,false);
						}
					}
				} else if(param.iniPath=="") {
					// initialize randomly
					initialize_matrix_randomly(semilla,gamma,val_ini,param.stdIni,false);
					initialize_matrix_randomly(semilla,beta,val_ini,param.stdIni,false);
					/*
					initialize_matrix_randomly(semilla,gamma,0.1/sqrt(param.flag_price),0.1,true);
					initialize_matrix_randomly(semilla,beta,0.1/sqrt(param.flag_price),0.1,true);
					*/
				} else {
					// initialize from file
					initialize_from_file(param.iniPath+"/param_gamma",data.user_ids,gamma,param,false);
					initialize_from_file(param.iniPath+"/param_beta",data.item_ids,beta,param,false);
				}
			}
		}
		...
		if(param.flag_day>0) {
			if(param.iniPath=="") {
				...
				// initialize from file
				initialize_from_file(param.iniPath+"/param_mu",data.item_ids,mu,param,false);
				initialize_from_file(param.iniPath+"/param_delta",data.day_ids,delta,param,false);
			}
		}

		// initialize per-weekday effect variables
		if(param.flag_weekdays) {
			if(param.iniPath=="") {
				// initialize randomly
				initialize_matrix_randomly(semilla,weekdays,val_ini,param.stdIni,false);
			} else {
				// initialize from file
				initialize_from_file(param.iniPath+"/param_weekdays",data.item_ids,weekdays,param,false);
			}
		}
	}
	...
	inline void set_to_one(Matrix2D<bool> &m) {
		for(int n=0; n<m.get_size1(); n++) {
			for(int k=0; k<m.get_size2(); k++) {
				m.set_object(n,k,true);
			}
		}
	}

	inline void set_to_zero(Matrix2D<bool> &m) {
		for(int n=0; n<m.get_size1(); n++) {
			for(int k=0; k<m.get_size2(); k++) {
				m.set_object(n,k,false);
			}
		}
	}
